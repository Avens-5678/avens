  ...
  if (submitted) {
    return (
      <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 text-center space-y-2">
        ...
        <p className="text-sm font-semibold text-foreground">Site Visit Requested!</p>
        <p className="text-xs text-muted-foreground">The venue owner will confirm your appointment shortly.</p>
        ...
        <div className="space-y-1">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className={cn("w-full justify-start text-left font-normal h-10 text-xs", !preferredDate && "text-muted-foreground")}>
                <CalendarIcon className="h-3.5 w-3.5 mr-1.5" />
                {preferredDate ? format(preferredDate, "dd MMM yyyy") : "Select date"}
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0" align="start">
              <Calendar mode="single" selected={preferredDate} onSelect={setPreferredDate} disabled={(d) => d < today} initialFocus className="p-3 pointer-events-auto" />
            </PopoverContent>
          </Popover>
        </div>
