...
interface RentalItemFormDialogProps {
  ...
  editingItem?: any;
  onSave: (data: Record<string, any>, variants: { attributeType: string; rows: VariantRow[] } | null) => Promise<void>;
  title?: string;
  /** Hide fields not relevant for vendor inventory (like rating, show_on_home, display_order) */
  vendorMode?: boolean;
}

const RentalItemFormDialog = ({ open, onOpenChange, editingItem, onSave, title, vendorMode }: RentalItemFormDialogProps) => {
    ...
    if (isOpen) {
      if (editingItem) {
        setFormData({
          ...
          price_value: editingItem.price_value ?? '',
          pricing_unit: editingItem.pricing_unit || 'Per Day',
  ...
  const loadVariants = async (itemId: string, tableName: string) => {
    const fkColumn = tableName === "vendor_inventory_variants" ? "inventory_item_id" : "rental_id";
  ...
  const addVariantRow = () => {
    ...
    setVariantRows(prev => [...prev, { attribute_value: newAttributeValue.trim(), price_value: null, pricing_unit: "Per Day", stock_quantity: 1, image_url: null }]);
    setNewAttributeValue("");
  };

  const removeVariantRow = (index: number) => setVariantRows(prev => prev.filter((_, i) => i !== index));
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{title || (editingItem ? "Edit Item" : "Create New Item")}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="space-y-3">
              ...
              {vendorMode && (
                <div className="space-y-1"><Label>Search Keywords</Label><Input value={formData.search_keywords || ''} onChange={(e) => setFormData(prev => ({ ...prev, search_keywords: e.target.value }))} placeholder="Comma-separated keywords" /></div>
              )}
              <div className="flex items-center gap-6">
                ...
                {!vendorMode && <div className="flex items-center gap-2"><Switch checked={formData.show_on_home !== false} onCheckedChange={(c) => setFormData(prev => ({ ...prev, show_on_home: c }))} /><Label>Home</Label></div>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 p-4 rounded-xl bg-muted/50 border border-border">
            ...
            <div>
              <Label className="text-sm font-medium">Does this item have variations?</Label>
              ...
              {variantRows.length > 0 && (
                <div className="border border-border rounded-xl overflow-hidden">
                  <div className="grid grid-cols-[1fr_100px_120px_80px_80px_40px] gap-2 p-3 bg-muted/50 text-xs font-semibold text-muted-foreground uppercase">
                    <span>{attributeType}</span><span>Price</span><span>Unit</span><span>Stock</span><span>Image</span><span></span>
                  </div>
        ...
        <div className="flex gap-2 pt-2">
          ...
          <Button onClick={() => onOpenChange(false)} variant="outline" className="flex-1"><X className="mr-2 h-4 w-4" />Cancel</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
...
export default RentalItemFormDialog;
