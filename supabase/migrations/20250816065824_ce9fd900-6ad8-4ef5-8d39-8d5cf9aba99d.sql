...
-- Insert sample hero banners (if not already present)
INSERT INTO hero_banners (title, subtitle, image_url, event_type, button_text)
SELECT * FROM (VALUES
  ('Creating Unforgettable Moments', 'Premium event management and rental services for your special occasions', 'https://images.unsplash.com/photo-1520637836862-4d197d17c86a?w=1920&h=1080&fit=crop', 'wedding', 'Explore Weddings'),
  ('Perfect Corporate Events', 'Professional event management that elevates your business image', 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=1920&h=1080&fit=crop', 'corporate', 'Learn More'),
  ('Magical Celebrations', 'Making every birthday and anniversary truly special', 'https://images.unsplash.com/photo-1464349095431-e9a21285b5f3?w=1920&h=1080&fit=crop', 'birthday', 'Plan Your Party')
) AS v(title, subtitle, image_url, event_type, button_text)
WHERE NOT EXISTS (SELECT 1 FROM hero_banners WHERE hero_banners.title = v.title);
